// import * as users_data from "../userdata/users.json"
import * as THREE from 'three'
import * as TWEEN from 'tween'
import $ from 'jquery';
import { users_data } from './users_data'
import pics from '../userdata/pic/*.png'
...
function newFetchedPic(index) {
  let pic_path = pics["Frame-" + index]
...
function getUsers(data) {
  USERS = data
  USERS.map((user_dict) => {
    +user_dict.pic >= 0 ? (user_dict.picLink = newFetchedPic(user_dict.pic)) : void null
  }
  );
    ...
    if (sectsWithPoints[0]) {
      ...
      if (RUNNING_INDEXES.indexOf(sectsWithPoints[0].index) == -1) {
        ...
        picindex < 61 ?
          picindex++
          :
          ((picindex = 0), (looped_picindex = true));
